	...
	case WM_INITDIALOG:
		{
			...
			len=256;
			err=RegQueryValueEx(key,"bind",NULL,NULL,(unsigned char*)server,&len);
			if(err!=ERROR_SUCCESS){
			...
			len=64;
			err=RegQueryValueEx(key,"logfile",NULL,NULL,(unsigned char*)name,&len);
			if(err!=ERROR_SUCCESS){
				strcpy(name,"capi20proxy.log");
			}
			...
			ListView_InsertColumn(GetDlgItem(hwnd,IDC_USERS),
				0,
				&col
				);
